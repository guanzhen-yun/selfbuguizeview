...
public class IrregularMenuView extends View {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        Path path = new Path();
        //内圆
        path.addCircle(cx, cy, innerCr - 2 * division, Path.Direction.CW);//顺时针方向
        ...
        path.lineTo((float) (cx + outerCr * Math.sin(sweepAngleOuter * Math.PI / 360)), (float) (cy - outerCr * Math.cos(sweepAngleOuter * Math.PI / 360)));
        path.addArc(outerRectF, -45 - disAngleOuter, -sweepAngleOuter);
        path.lineTo((float) (cx - innerCr * Math.sin(sweepAngleInner * Math.PI / 360)), (float) (cy - innerCr * Math.cos(sweepAngleInner * Math.PI / 360)));
